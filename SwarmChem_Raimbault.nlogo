...
to setup-agents
  ...
  ;;read the table describing species and setup agents
  foreach species-ids [
...
;;agent procedure but used at setup so here.
to new-agent [species-id]
  ;;inevitably go other all params for each agent
  ;;shouldn't be costly
...
to-report spatial-autocorrelation-index [species-id]
  ...
  clear-table-moran
  let ag sort-by [lexcomp ?1 ?2 (list task [floor (xcor / clustering-grid-size)] task [floor (ycor / clustering-grid-size)])] (agents with [species = species-id])
  let min-ag first ag
